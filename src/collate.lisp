...
(defmethod backend-collate ((backend icu-backend) collator string-a string-b)
  ...
  (with-foreign-object (err :int)
    ...
    (with-foreign-strings ((a string-a :encoding :utf-8)
      ...
      (let ((r (cl-stack-icu:ucol-strcoll-utf8 (l10n-protocol::collator-raw collator)
                                               a -1 b -1 err)))
   ...
   (lambda (src src-len)
     (let ((cap (max 64 (* src-len 4))))
       (with-foreign-pointer (buf cap)
         (let ((n (cl-stack-icu:ucol-get-sort-key (l10n-protocol::collator-raw collator)
                                                  src src-len buf cap)))
           (when (> n cap)
             (with-foreign-pointer (buf2 n)
               (setf n (cl-stack-icu:ucol-get-sort-key (l10n-protocol::collator-raw collator)
                                                       src src-len buf2 n))
